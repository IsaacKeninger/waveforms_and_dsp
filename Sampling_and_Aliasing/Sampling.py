...
# Chat GPT Prompt: codebase + ho"w to make each signal the same length no matter what" made this function
...
def lowpassFilter(signal, fs,cutoffFreq):
    num_taps = 91 # number of filter coefficents
    Impulse_Response = scipy.signal.firwin(num_taps, cutoffFreq, fs=fs)  # Design FIR Filter Via given cutoffFrequency and sammple rate and num of taps. Returns Impulse Response
    # Array of 91 weights is returned
    filteredSignal = scipy.signal.lfilter(Impulse_Response,[1.0],signal) # Applies Discrete Convolution. 1.0 argument means there is no feedback
    # ex. y[n] = (b0 * x[n]) + (b1 * x[n-1]) + ... + (Bmax x[n-max])
    # Any n < 0 are assumed to be 0, this is the build up
